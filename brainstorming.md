...
| テーブル数 | 20〜30テーブル |
| 最大行数 | 3,000行/テーブル |
| データ入力 | CSVファイルによるテーブル単位の一括インポート |
...
### 概要
テーブルごとにCSVファイルをアップロードしてデータを一括登録できる。

### 仕様

| 項目 | 内容 |
|------|------|
| 対象ロール | Admin・Editor（Viewerは不可） |
| 単位 | テーブル1つにつきCSV1ファイル |
| ヘッダー | 1行目をカラム名として扱う |
| 重複処理 | 選択式（上書き / スキップ / 追記）|
| エラー処理 | バリデーションエラー行を一覧表示し、正常行のみインポートするか中断するか選択 |

### 実装方針
- フロント（Vue.js）：ファイル選択 → プレビュー表示 → インポート実行
- バック（Spring Boot）：CSVパース → バリデーション → DB書き込み
- ライブラリ候補：Apache Commons CSV / OpenCSV
...
| 項目 | 決定内容 |
|------|---------|
| RDBMS | **PostgreSQL**（開発: SQLite → 本番: AWS RDS PostgreSQL） |
| テーブル定義 | **自動認識**（DBスキーマを読み取り、全テーブルを一覧表示） |
| 外部ユーザー管理 | **招待制**（管理者がメールで招待、自己登録不可） |

## ネクストアクション

- [x] RDBMSをMySQL / PostgreSQLどちらにするか決定 → PostgreSQL
- [x] テーブル定義の自動認識 or 手動設定の方針決定 → 自動認識
- [x] 外部ユーザーのアカウント管理方式を決定 → 招待制
